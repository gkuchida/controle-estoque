...
export class DetalhesProdutoComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    if(produtos){
      ...
      if(this.produtoDetalhado && this.produtoDetalhado?.data){
        this.selectedDate = format(new Date(this.produtoDetalhado.data), 'dd/MM/yyyy');
      }
    }
  }
  getItemNome(value: string): string{
    const item = this.itemNomes.find(item => item.value === value);
  ...
  addLeadingZero(value: number): string{
    return value < 10 ? `O${value}` : value.toString();
  }
}
